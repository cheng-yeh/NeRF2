    ...
    def eval_network_rssi(self):
        ...
        with torch.no_grad():
            with open(os.path.join(self.logdir, self.expname, "result.txt"), 'w') as f:
                self.logger.info(f"Writing result to {os.path.join(self.logdir, self.expname, 'result.txt')}...")
                for test_input, test_label in self.test_iter:
                    test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
        ...
        with torch.no_grad():
            with open(os.path.join(self.logdir, self.expname, "result_pos.txt"), 'w') as f:
                self.logger.info(f"Writing result to {os.path.join(self.logdir, self.expname, 'result_pos.txt')}...")
                for test_input, test_label in self.test_iter:
                    test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
                    tx_o, rays_o, rays_d = test_input[:, :3], test_input[:, 3:6], test_input[:, 6:]
                    ...
                    for i, rssi in enumerate(predict_rssi):
                        f.write(f"{tx_o[i][0].item()}, {tx_o[i][1].item()}, {tx_o[i][2].item()}\n")
